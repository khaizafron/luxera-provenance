...
export interface PortfolioClient {
  ...
  updated_at: string;
  last_case_id?: string;
  last_case_status?: CaseStatus | DecisionStatus | 'NO_CASE';
}
...
export interface SoWCase {
  ...
  portfolio_client_name?: string;
  portfolio_total_deposited?: number;
...
class Store {
  ...
  private _portfolioClients: PersistentMap<string, PortfolioClient>;
  ...
  constructor() {
    ...
    this._portfolioClients = new PersistentMap(null, onChange);
  ...
  get portfolioClients() { this.syncFromDisk(); return this._portfolioClients; }
  ...
  set portfolioClients(val) { this._portfolioClients = val; this.saveState(); }
    ...
    try {
      ...
      if (fs.existsSync(bakPath)) {
        try {
          ...
          this._mergeMap(this._organizations, parsed.organizations);
          this._mergeMap(this._users, parsed.users);
          this._mergeMap(this._portfolioClients, parsed.portfolioClients);
          this._mergeMap(this._cases, parsed.cases);
          this._mergeMap(this._documents, parsed.documents);
          this._mergeMap(this._jobs, parsed.jobs);
          this._mergeAuditBlocks(parsed.auditBlocks);
  ...
  public saveState() {
    if (this.isLoadingState || this.isSavingState) return;
    ...
    try {
      const serialized = {
        ...
        portfolioClients: Array.from(this._portfolioClients.entries()),
  ...
  public getPortfolioClient(orgId: string, clientId: string): PortfolioClient | undefined {
    this.syncFromDisk();
    return Array.from(this._portfolioClients.values()).find(
      (client) => client.organization_id === orgId && client.client_id === clientId
    );
  }

  public getOrganizationPortfolioClients(orgId: string): PortfolioClient[] {
    this.syncFromDisk();
    return Array.from(this._portfolioClients.values()).filter((client) => client.organization_id === orgId);
  }
  ...
  private seedDefaults() {
    ...
    const org: Organization = {
      id: 'ORG-LUXERA-01',
      name: 'Luxera Cognitive Resources',
      license_type: 'Source of Wealth Compliance License',
