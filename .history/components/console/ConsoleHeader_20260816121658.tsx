...
export function ConsoleHeader({ onOpenMobile }: ConsoleHeaderProps) {
  ...
  const statusLabel = (
    <span className="flex items-center gap-1.5 whitespace-nowrap">
      <span className="hidden sm:inline">PDPA 2010</span>
      <span className="hidden sm:inline text-slate-400">•</span>
      <span className="hidden sm:inline">PII Redaction Active</span>
      <span className="sm:hidden">PII Active</span>
    </span>
  );
  ...
  return (
    <header className="h-16 bg-[#0f1013]/90 backdrop-blur-md border-b border-slate-800 px-4 md:px-8 flex items-center justify-between shrink-0 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3 text-xs">
        <span className="ui-header-status ui-pill--white" aria-label="PDPA 2010 PII Redaction Active">
