...
function NewCasePageContent() {
  ...
  useEffect(() => {
    const portfolioClientId = searchParams.get('client_id');
    const portfolioClientName = searchParams.get('client_name');
    const portfolioCurrency = searchParams.get('currency');

    if (portfolioClientId) {
      setCustomerName(portfolioClientName || '');
      setCurrency(portfolioCurrency || 'MYR');
    }
  }, [searchParams]);
    ...
    try {
      const res = await fetch('/api/cases', {
        ...
        body: JSON.stringify({
          ...
          portfolio_client_id: searchParams.get('client_id') || undefined,
          portfolio_client_name: searchParams.get('client_name') || undefined,
          portfolio_total_deposited: searchParams.get('total_deposited') || undefined,
          portfolio_currency: searchParams.get('currency') || undefined,
        }),
      });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      {searchParams.get('client_id') && (
        <div className="rounded-xl border border-amber-500/40 bg-gradient-to-r from-amber-500/12 to-amber-500/6 px-4 py-3 text-xs text-amber-50 shadow-[0_8px_18px_-14px_rgba(251,191,36,0.8)] space-y-3">
          <div className="font-medium text-amber-200 uppercase tracking-[0.18em] text-[10px]">Portfolio Context Attached</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 text-[11px]">
            <div className="rounded-lg border border-slate-800/80 bg-[#0a0d12]/80 px-3 py-2">
              <div className="text-slate-400 font-mono uppercase text-[9px]">Client ID</div>
              <div className="mt-1 font-medium text-white">{searchParams.get('client_id')}</div>
            </div>
            <div className="rounded-lg border border-slate-800/80 bg-[#0a0d12]/80 px-3 py-2">
              <div className="text-slate-400 font-mono uppercase text-[9px]">Client Name</div>
              <div className="mt-1 font-medium text-white">{searchParams.get('client_name')}</div>
            </div>
            <div className="rounded-lg border border-slate-800/80 bg-[#0a0d12]/80 px-3 py-2">
              <div className="text-slate-400 font-mono uppercase text-[9px]">Portfolio Exposure</div>
              <div className="mt-1 font-medium text-emerald-400">
                {searchParams.get('currency')} {searchParams.get('total_deposited') ? Number(searchParams.get('total_deposited')).toLocaleString() : '0'}
              </div>
            </div>
            <div className="rounded-lg border border-slate-800/80 bg-[#0a0d12]/80 px-3 py-2">
              <div className="text-slate-400 font-mono uppercase text-[9px]">Currency</div>
              <div className="mt-1 font-medium text-white">{searchParams.get('currency') || 'MYR'}</div>
            </div>
          </div>
        </div>
      )}
                ...
                {uploadedDocs.map((doc) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="ui-status ui-status--green">
                        OCR: COMPLETED
                      </span>
...
export default function NewCasePage() {
  return (
    <Suspense fallback={<div className="max-w-4xl mx-auto py-12 text-slate-300">Loading case creator…</div>}>
      <NewCasePageContent />
    </Suspense>
  );
}
