  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      {searchParams.get('client_id') && (
        <div className="rounded-xl border border-amber-500/40 bg-gradient-to-r from-amber-500/12 to-amber-500/6 px-4 py-3 text-xs text-amber-50 shadow-[0_8px_18px_-14px_rgba(251,191,36,0.8)]">
          Portfolio context attached: {searchParams.get('client_id')} • {searchParams.get('client_name')} • {searchParams.get('currency')} • {searchParams.get('total_deposited') ? `Exposure: ${searchParams.get('currency')} ${Number(searchParams.get('total_deposited')).toLocaleString()}` : 'Portfolio context available'}
        </div>
      )}
