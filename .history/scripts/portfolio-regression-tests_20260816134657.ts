...
async function main() {
  ...
  console.log('TEST 7: Portfolio Case - High Risk / Bank Deposit Inconsistency');
  ...
  // The portfolio exposure (2.4M) is supported by bank deposits (3.6M)
  // But the bank deposits themselves are inconsistent with declared income (180K)
  // This is caught by the existing SALARY_VS_DEPOSIT_RATIO rule, not the portfolio rule
  ...
  await expect(
    ...
    'Portfolio consistency rule should pass: Portfolio exposure IS supported by bank deposits'
  );
  
  // The high-risk decision should come from the existing deposit/income ratio rule
  const ratioRule = highRiskResult.rule_evaluation_results.find((r) => r.rule_id === 'RULE_SALARY_VS_DEPOSIT_RATIO');
  await expect(
    ratioRule && !ratioRule.passed,
    'Existing ratio rule should fail: Bank deposits (3.6M) inconsistent with income (180K)'
  );
  
  // Verify high-risk decision is due to the deposit/income discrepancy
  await expect(
    highRiskResult.overall_decision === 'REJECTED',
    'High deposit/income ratio should produce REJECTED decision'
  );
  
  await expect(
    highRiskResult.composite_risk_score >= 50,
    'High risk portfolio case with deposit/income mismatch should produce score >= 50'
  );
  ...
  console.log(`  Deposit/Income Ratio: ${(highRiskResult.extracted_financial_profile.total_bank_deposits_detected! / highRiskCase.declared_annual_income).toFixed(2)}x`);
  console.log(`  Risk Score: ${highRiskResult.composite_risk_score}`);
  console.log(`  Decision: ${highRiskResult.overall_decision}`);
  console.log(`  Portfolio Consistency Rule Passed: ${portfolioConsistencyRule?.passed}`);
  console.log('✓ PASS\n');
