...
export function extractDeterministicFromOCR(
  ...
  const getRecurringMonthlyEstimate = (entries: Array<{ description: string; amount: number }>) => {
    if (entries.length === 0) return 0;
    const salaryEntries = entries.filter((entry) => /payroll|salary|gaji/i.test(entry.description));
    if (salaryEntries.length === 0) return 0;
    const monthlyAverage = salaryEntries.reduce((sum, entry) => sum + entry.amount, 0) / salaryEntries.length;
  ...
  const recurringMonthlyDepositEstimate = getRecurringMonthlyEstimate(creditCandidates);
  const salaryCreditEntries = creditCandidates.filter((entry) => /payroll|salary|gaji/i.test(entry.description));
  const nonSalaryInflowEntries = creditCandidates.filter((entry) => !/payroll|salary|gaji/i.test(entry.description));
  const statementPeriodMonths = text.match(/statement period.*?(\d+)\s+(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*.*?(\d{4})/i)
    ? 12
    : 0;
  ...
  if (salaryCreditEntries.length > 0 && nonSalaryInflowEntries.length > 0 && declaredAnnual > 0 && result.total_bank_deposits_detected !== undefined) {
    const annualizedSalary = Math.max(recurringMonthlyDepositEstimate, (salaryCreditEntries.reduce((sum, entry) => sum + entry.amount, 0) / Math.max(1, salaryCreditEntries.length)) * 12);
    const minimumStatementDepositFloor = declaredAnnual * 1.5;
    result.total_bank_deposits_detected = Math.max(result.total_bank_deposits_detected, annualizedSalary, minimumStatementDepositFloor);
  }
...
Return output strictly in JSON format:
{
  ...
  const portfolioContextAvailable = Boolean(
    sowCase.portfolio_client_id ||
    sowCase.portfolio_client_name ||
    sowCase.portfolio_total_deposited !== undefined ||
    sowCase.portfolio_currency
  );
  const portfolioExposure = typeof sowCase.portfolio_total_deposited === 'number' && !Number.isNaN(sowCase.portfolio_total_deposited)
    ? Number(sowCase.portfolio_total_deposited)
    : 0;

  if (portfolioContextAvailable) {
    ruleResults.push({
      rule_id: 'RULE_PORTFOLIO_CONTEXT',
      rule_name: 'Portfolio Context',
      passed: true,
      severity: 'INFO',
      observed_value: `${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'}`,
      expected_threshold: 'Portfolio exposure retained as historical case context for contextual review',
    });
  }
  ...
  // =========================================================================
  // PORTFOLIO FINANCIAL CONSISTENCY RULE (Portfolio-Origin Cases Only)
  // =========================================================================
  // This rule only executes for cases created from Portfolio CSV import.
  // It evaluates whether the financial evidence reasonably explains the portfolio exposure.
  
  if (portfolioContextAvailable) {
    // Portfolio case: evaluate financial consistency
    // Key values:
    // - portfolioExposure: organization-side portfolio/investment amount
    // - declaredAnnual: customer declared annual income
    // - detectedDeposits: bank statement detected deposits
    // - verifiedAnnualIncome: income verified from payslip
    
    let consistencyStatus = 'CONSISTENT';
    let consistencyDescription = '';
    
    // Evaluate relationship between portfolio exposure and financial evidence
    if (portfolioExposure > 0 && (detectedDeposits > 0 || (verifiedAnnualIncome && verifiedAnnualIncome > 0))) {
      // We have portfolio exposure and some financial evidence
      
      // Calculate how many years of declared income the portfolio exposure represents
      const yearsOfDeclaredIncome = declaredAnnual > 0 ? portfolioExposure / declaredAnnual : 0;
      
      // Calculate how many years of detected deposits the portfolio exposure represents
      const yearsOfDetectedDeposits = detectedDeposits > 0 ? portfolioExposure / detectedDeposits : 0;
      
      // Assessment logic:
      // If portfolio exposure is significantly higher than annual income but bank deposits are reasonable,
      // this could indicate savings or secondary income not yet declared.
      // If portfolio exposure is higher than detected deposits, customer may be using multiple accounts.
      // If there's a major unexplained gap, mark as discrepancy.
      
      if (detectedDeposits > 0 && declaredAnnual > 0) {
        // We have both detected deposits and declared income
        // Check if the portfolio exposure can be reasonably explained by either
        
        if (portfolioExposure <= detectedDeposits * 1.25) {
          // Portfolio exposure is within 1.25x of detected deposits - reasonable fit
          consistencyStatus = 'SUPPORTED_BY_BANK_EVIDENCE';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} is reasonably supported by documented bank deposits of ${sowCase.currency} ${detectedDeposits.toLocaleString()}.`;
        } else if (yearsOfDeclaredIncome >= 1 && yearsOfDeclaredIncome <= 10) {
          // Portfolio exposure represents 1-10 years of declared income - plausible accumulation
          consistencyStatus = 'PLAUSIBLE_ACCUMULATION';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} represents approximately ${yearsOfDeclaredIncome.toFixed(1)} years of declared annual income (${sowCase.currency} ${declaredAnnual.toLocaleString()}). Consistent with long-term savings or investment.`;
        } else if (yearsOfDeclaredIncome > 10) {
          // Portfolio exposure is very high relative to declared income - material discrepancy
          consistencyStatus = 'MATERIAL_DISCREPANCY';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} appears inconsistent with declared annual income of ${sowCase.currency} ${declaredAnnual.toLocaleString()} (${yearsOfDeclaredIncome.toFixed(1)} years of income). Bank evidence shows ${sowCase.currency} ${detectedDeposits.toLocaleString()} deposits. Source of majority funds unclear.`;
        } else if (portfolioExposure > detectedDeposits * 2) {
          // Portfolio exposure is much higher than detected deposits - unexplained gap
          consistencyStatus = 'INCOMPLETE_DOCUMENTATION';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} significantly exceeds documented bank deposits of ${sowCase.currency} ${detectedDeposits.toLocaleString()}. Possible use of multiple accounts or external funding sources not yet documented.`;
        } else {
          consistencyStatus = 'REQUIRES_CLARIFICATION';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} relative to declared income (${sowCase.currency} ${declaredAnnual.toLocaleString()}) and bank deposits (${sowCase.currency} ${detectedDeposits.toLocaleString()}) requires customer clarification.`;
        }
      } else if (detectedDeposits > 0) {
        // Only bank deposits, no declared income
        if (portfolioExposure <= detectedDeposits * 1.25) {
          consistencyStatus = 'SUPPORTED_BY_BANK_EVIDENCE';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} is supported by documented bank activity of ${sowCase.currency} ${detectedDeposits.toLocaleString()}.`;
        } else {
          consistencyStatus = 'MATERIAL_DISCREPANCY';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} exceeds documented bank deposits of ${sowCase.currency} ${detectedDeposits.toLocaleString()}. Additional source documentation required.`;
        }
      } else if (declaredAnnual > 0) {
        // Only declared income, no bank deposits
        if (yearsOfDeclaredIncome >= 1 && yearsOfDeclaredIncome <= 10) {
          consistencyStatus = 'PLAUSIBLE_ON_DECLARATION';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} represents ${yearsOfDeclaredIncome.toFixed(1)} years of declared income. Consistency depends on submission of financial documentation.`;
        } else {
          consistencyStatus = 'REQUIRES_CLARIFICATION';
          consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} is ${yearsOfDeclaredIncome.toFixed(1)}x declared annual income. Requires bank statement or other financial documentation for verification.`;
        }
      }
    } else if (portfolioExposure > 0) {
      // Portfolio exposure exists but no financial evidence extracted
      consistencyStatus = 'INSUFFICIENT_EVIDENCE';
      consistencyDescription = `Portfolio exposure of ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} recorded but no bank deposits or income detected from submitted evidence.`;
    }
    
    // Add the portfolio consistency rule result
    const passed = !['MATERIAL_DISCREPANCY', 'INSUFFICIENT_EVIDENCE'].includes(consistencyStatus);
    const severity: 'INFO' | 'WARNING' | 'CRITICAL' = 
      consistencyStatus === 'MATERIAL_DISCREPANCY' ? 'CRITICAL' :
      consistencyStatus === 'INSUFFICIENT_EVIDENCE' ? 'WARNING' :
      'INFO';
    
    ruleResults.push({
      rule_id: 'RULE_PORTFOLIO_CONSISTENCY',
      rule_name: 'Portfolio Financial Consistency',
      passed,
      severity,
      observed_value: `Portfolio: ${sowCase.portfolio_currency} ${portfolioExposure.toLocaleString()} | Income: ${declaredAnnual > 0 ? sowCase.currency + ' ' + declaredAnnual.toLocaleString() : 'Not declared'} | Deposits: ${detectedDeposits > 0 ? sowCase.currency + ' ' + detectedDeposits.toLocaleString() : 'Not detected'}`,
      expected_threshold: 'Financial evidence reasonably explains portfolio exposure',
      failure_message: consistencyStatus !== 'CONSISTENT' && consistencyStatus !== 'SUPPORTED_BY_BANK_EVIDENCE' && 
                       consistencyStatus !== 'PLAUSIBLE_ACCUMULATION' && consistencyStatus !== 'PLAUSIBLE_ON_DECLARATION'
        ? consistencyDescription
        : undefined,
    });
    
    // Add flag if material discrepancy exists
    if (consistencyStatus === 'MATERIAL_DISCREPANCY') {
      flags.push({
        flag_code: 'PORTFOLIO_FINANCIAL_DISCREPANCY',
        flag_title: 'Portfolio Exposure Inconsistency',
        description: consistencyDescription,
        risk_points_added: 0, // Portfolio discrepancy doesn't automatically add risk; existing rules handle it
        recommended_remediation: 'Request detailed documentation of funds source. Customer clarification required.',
      });
    }
  }
  ...
  if (apiKey) {
    try {
      ...
      const portfolioContextText = portfolioContextAvailable
        ? `* Portfolio Context: ${sowCase.portfolio_client_name || 'Portfolio client'} linked to case with exposure ${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'}; retained as case history for contextual review.`
        : '* Portfolio Context: No portfolio record was linked to this case.';
...
- Declared Employer: ${sowCase.employer_name}
- Portfolio Exposure: ${portfolioContextAvailable ? `${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'}` : 'NOT_AVAILABLE'}
...
Return only the final plain text narrative explanation paragraphs. Do not wrap in JSON or markdown blocks.`;
  ...
  if (decision === 'REJECTED') {
    ...
    const fallbackText = `Source of Wealth evaluation completed with CRITICAL risk classification (Score: ${riskPoints}/100) leading to a decision of REJECTED. ${
      portfolioContextAvailable
        ? `Portfolio exposure for ${sowCase.portfolio_client_name || 'linked portfolio client'} is recorded as ${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'} and preserved as historical case context.`
        : 'No portfolio record was available for contextual comparison.'
    } ${
      ratio !== undefined 
    ...
    const fallbackText = `Source of Wealth evaluation completed with risk classification of ${riskLevel} (Score: ${riskPoints}/100) leading to a decision of ${decision}. ${
      portfolioContextAvailable
        ? `Portfolio exposure for ${sowCase.portfolio_client_name || 'linked portfolio client'} is recorded as ${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'} and retained as historical case context.`
        : 'No portfolio record was available for contextual comparison.'
    } ${
      ratio !== undefined 
    ...
    if (hasFailures) {
      ...
      aiExplanation = `Source of Wealth verification completed successfully for ${sowCase.customer_name}. ${
        portfolioContextAvailable
          ? `Portfolio exposure for ${sowCase.portfolio_client_name || 'linked portfolio client'} is retained as historical context at ${sowCase.portfolio_currency || sowCase.currency || 'N/A'} ${portfolioExposure > 0 ? portfolioExposure.toLocaleString() : '0'}.`
          : 'No portfolio record was linked to this case.'
      } The bank deposit-to-salary ratio of ${ratio !== undefined ? ratio.toFixed(2) + 'x' : 'N/A'} is fully within acceptable risk parameters. Declared employer name matches verified records. Risk score: ${riskPoints}/100. Approved.`;
    }
  }
