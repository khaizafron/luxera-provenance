...
export function extractDeterministicFromOCR(
  ...
  const getTransactionCreditCandidates = (lines: string[]): Array<{ description: string; amount: number }> => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const descriptorIndex = block.findIndex((entry) => isCreditDescriptor(entry));
      const creditLabelIndex = block.findIndex((entry) => /credit\s*\(myr\)|credit\s*\:|credit/i.test(entry));

      const selectFromBlock = () => {
        const amountCandidates: Array<{ description: string; amount: number }> = [];
        for (let idx = 0; idx < block.length; idx++) {
          const current = block[idx];
          if (!current || isDateLine(current) || isSummaryLine(current)) continue;
          if (isDebitDescriptor(current) || /balance/i.test(current)) continue;

          const values = extractMoneyValues(current);
          if (values.length === 0) continue;

          const quote = current.replace(/\s+/g, ' ').trim();
          if (isCreditDescriptor(quote) || quote.toLowerCase().includes('credit')) {
            const amount = values[0];
            if (amount > 0) {
              amountCandidates.push({ description: quote, amount });
            }
          }
        }
      ...
      const blockCandidates = selectFromBlock();
      for (const candidate of blockCandidates) {
        const key = `${candidate.description}|${candidate.amount}`;
        if (!seen.has(key)) {
          seen.add(key);
      ...
      if (descriptorIndex >= 0) {
        for (let idx = descriptorIndex + 1; idx < block.length; idx++) {
          const current = block[idx].trim();
          if (!current || isDateLine(current) || isSummaryLine(current)) continue;
          if (isDebitDescriptor(current) || /balance/i.test(current)) break;
          const values = extractMoneyValues(current);
          if (values.length > 0) {
            const [amount] = values;
            if (amount > 0) {
              const key = `${block[descriptorIndex]}|${amount}`;
              if (!seen.has(key)) {
                seen.add(key);
                candidates.push({ description: block[descriptorIndex], amount });
              }
            }
            break;
          }
      ...
      if (creditLabelIndex >= 0) {
        for (let idx = creditLabelIndex + 1; idx < block.length; idx++) {
          const current = block[idx].trim();
          if (!current || isDateLine(current) || isSummaryLine(current)) continue;
          if (isDebitDescriptor(current) || /balance/i.test(current)) break;
          const values = extractMoneyValues(current);
          if (values.length > 0) {
            const [amount] = values;
            if (amount > 0) {
              const key = `table|${amount}`;
              if (!seen.has(key)) {
                seen.add(key);
                candidates.push({ description: 'Table credit entry', amount });
              }
            }
            break;
          }
