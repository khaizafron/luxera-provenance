...
export function extractDeterministicFromOCR(
  ...
  if (salaryCreditEntries.length > 1 && declaredAnnual > 0 && result.total_bank_deposits_detected !== undefined) {
    const annualizedSalary = Math.max(recurringMonthlyDepositEstimate, (salaryCreditEntries.reduce((sum, entry) => sum + entry.amount, 0) / salaryCreditEntries.length) * 12);
