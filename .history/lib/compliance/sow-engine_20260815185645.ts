...
export function extractDeterministicFromOCR(
  ...
  const getTransactionCreditCandidates = (lines: string[]): Array<{ description: string; amount: number }> => {
    ...
    const pushCandidate = (description: string, amount: number) => {
      ...
      const key = `${cleanDescription}|${amount}`;
      if (!seen.has(key)) {
        seen.add(key);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('|')) {
        ...
        if (parts.length >= 3) {
          ...
          if (amount !== null && amount > 0 && !/debit|withdraw|payment|card\s*\/|online|household|balance|expense|fee/i.test(description)) {
            pushCandidate(description, amount);
          }
        }
        continue;
      }
      ...
      for (let idx = 0; idx < block.length; idx++) {
        ...
        if (!entry || isDateLine(entry) || isSummaryLine(entry)) continue;

        const entryLower = entry.toLowerCase();
        const previous = idx > 0 ? block[idx - 1].trim() : '';
        const previousLower = previous.toLowerCase();
        const hasDebitMarkers = /debit|withdraw|payment|card\s*\/|online|household|expense|fee|balance/i.test(entryLower) || /debit|withdraw|payment|card\s*\/|online|household|expense|fee|balance/i.test(previousLower);
        const isCreditEntry = isCreditDescriptor(entryLower) || isCreditDescriptor(previousLower) || /salary|transfer|deposit|redemption|incoming/i.test(entryLower) || /salary|transfer|deposit|redemption|incoming/i.test(previousLower);

        if (hasDebitMarkers) continue;

        const values = extractMoneyValues(entry);
        if (values.length === 0) continue;

        const amount = values[0];
        if (amount > 0 && isCreditEntry) {
          candidateAmount = amount;
          candidateDescription = previousLower && isCreditDescriptor(previousLower) ? previous : entry;
          break;
        }
      }

      if (candidateAmount !== null && candidateAmount > 0) {
        pushCandidate(candidateDescription || 'Credit entry', candidateAmount);
      }
    }
