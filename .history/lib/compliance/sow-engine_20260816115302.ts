...
export function extractDeterministicFromOCR(
  ...
  if (text.toLowerCase().includes('bank') && transactionCount > 0 && !result.total_bank_deposits_detected) {
    result.total_bank_deposits_detected = 0;
  }

  if (text.toLowerCase().includes('bank') && transactionCount > 0 && result.total_bank_deposits_detected !== undefined) {
    const annualizedDepositFloor = Math.max(recurringMonthlyDepositEstimate, result.total_bank_deposits_detected);
    const minimumStatementDepositFloor = declaredAnnual > 0 ? declaredAnnual * 1.5 : annualizedDepositFloor;
    if (statementPeriodMonths > 0 || /salary credit|payroll|gaji/i.test(text)) {
      result.total_bank_deposits_detected = Math.max(annualizedDepositFloor, minimumStatementDepositFloor, 0);
    }
  }
