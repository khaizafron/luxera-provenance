...
export function extractDeterministicFromOCR(
  ...
  const isDateLine = (value: string): boolean => {
    ...
    return /(?:\b(?:19|20)\d{2}[-/][01]?\d[-/][0-3]?\d\b|\b\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(?:19|20)\d{2}\b)/i.test(trimmed);
  };

  const isCreditDescriptor = (value: string): boolean => {
    if (!value) return false;
    return /(salary\s+credit|credit|incoming\s+transfer|transfer|deposit|unit\s+trust\s+redemption|fixed\s+deposit\s+maturity|cash\s+in|credit\s*\()/i.test(value);
  };

  const isDebitOrBalanceDescriptor = (value: string): boolean => {
    if (!value) return false;
    return /(debit|withdraw|payment|card\s*\/|online|household|balance|purchase|fee|cash\s+out|atm|transfer\s+out)/i.test(value);
  };

  const getTransactionCreditCandidates = (lines: string[]): Array<{ date: string; description: string; amount: number }> => {
    const candidates: Array<{ date: string; description: string; amount: number }> = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const block = [line];
      const maxLookahead = Math.min(lines.length, i + 8);
      for (let j = i + 1; j < maxLookahead; j++) {
        const nextLine = lines[j].trim();
        if (!nextLine) continue;
        if (isDateLine(nextLine) || /statement summary|total credits|total deposits|balance carried|closing balance|opening balance/i.test(nextLine)) {
          break;
        }
        block.push(nextLine);
      }
      ...
      const dateMatch = blockText.match(/(?:\b(?:19|20)\d{2}[-/][01]?\d[-/][0-3]?\d\b|\b\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(?:19|20)\d{2}\b)/i);
      const dateValue = dateMatch ? dateMatch[0].trim() : '';
      let lastDescriptor = '';
      let lastDescriptorIndex = -1;

      for (let currentIndex = 0; currentIndex < block.length; currentIndex++) {
        const blockLine = block[currentIndex].trim();
        if (!blockLine) continue;

        const normalizedLine = blockLine.replace(/[\u00A0\t]+/g, ' ').trim();
        const hasAmount = /\d/.test(normalizedLine);
        if (isCreditDescriptor(normalizedLine)) {
          lastDescriptor = normalizedLine;
          lastDescriptorIndex = currentIndex;
          continue;
        }
        if (isDebitOrBalanceDescriptor(normalizedLine)) {
          lastDescriptor = '';
        ...
        const moneyMatches = [...normalizedLine.matchAll(/\d{1,3}(?:,\d{3})*(?:\.\d+)?|\d+(?:\.\d+)?/g)];
        if (moneyMatches.length === 0) continue;

        const moneyValues = moneyMatches.map((match) => parseMoney(match[0])).filter((amount): amount is number => amount !== null);
        if (moneyValues.length === 0) continue;

        const candidateAmount = moneyValues[0];
        const descriptorContext = currentIndex > 0 ? block.slice(Math.max(0, currentIndex - 2), currentIndex).join(' ') : '';
        const relevantDescriptor = lastDescriptor || descriptorContext;
        const lineLooksLikeBalance = /balance/i.test(normalizedLine) || /debit/i.test(normalizedLine);
        const shouldCount = (!lineLooksLikeBalance) && (isCreditDescriptor(relevantDescriptor) || isCreditDescriptor(normalizedLine));

        if (shouldCount && candidateAmount > 0) {
          const candidateKey = `${dateValue}|${(lastDescriptor || descriptorContext || normalizedLine).slice(0, 160)}|${candidateAmount}`;
          if (!seen.has(candidateKey)) {
            seen.add(candidateKey);
            candidates.push({ date: dateValue, description: lastDescriptor || descriptorContext || normalizedLine, amount: candidateAmount });
          }
        }
      ...
      if (blockText.includes('Credit (MYR)') || blockText.includes('CREDIT (MYR)')) {
        const blockDollarMatches = [...blockText.matchAll(/\d{1,3}(?:,\d{3})*(?:\.\d+)?|\d+(?:\.\d+)?/g)];
        const values = blockDollarMatches.map((match) => parseMoney(match[0])).filter((amount): amount is number => amount !== null);
        if (values.length >= 2) {
          const creditAmount = values[0];
          const balanceLike = values[1];
          const key = `${dateValue}|credit-table|${creditAmount}`;
          if (!seen.has(key) && creditAmount > 0 && balanceLike > creditAmount) {
            seen.add(key);
            candidates.push({ date: dateValue, description: 'Table credit entry', amount: creditAmount });
          }
        }
      }
    }
  ...
  for (const line of lines) {
    ...
    if (
      candidate.toUpperCase().includes('SDN BHD') ||
      candidate.toUpperCase().includes('BERHAD') ||
      candidate.toUpperCase().includes('BHD') ||
      candidate.toUpperCase().includes('LTD') ||
      candidate.toUpperCase().includes('CORP') ||
      candidate.toUpperCase().includes('INC') ||
      candidate.toUpperCase().includes('HOLDINGS') ||
      candidate.toUpperCase().includes('SERVICES') ||
      candidate.toUpperCase().includes('SOLUTIONS') ||
      candidate.toUpperCase().includes('TECHNOLOGY') ||
      candidate.toUpperCase().includes('TECHNOLOGIES') ||
      (declaredEmployer && candidate.toLowerCase().includes(declaredEmployer.toLowerCase().split(' ')[0]))
    ) {
      result.detected_employer_name = candidate;
      break;
    }
  }
  ...
  // Prefer transaction table sum if found
  if (transactionCount > 0) {
    result.total_bank_deposits_detected = sumTotalCredits;
    if (sumPayrollCredits > 0) {
