  ...
  return (
    <header className="h-16 bg-[#0f1013]/90 backdrop-blur-md border-b border-slate-800 px-4 md:px-8 flex items-center justify-between shrink-0 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3 text-xs">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/90 text-emerald-400 border border-emerald-500/30 font-mono text-[11px]">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
          <span className="hidden sm:inline font-medium">PDPA 2010 • PII Redaction Active</span>
          <span className="sm:hidden font-medium">PII Active</span>
        </span>
      </div>
    </header>
  );
